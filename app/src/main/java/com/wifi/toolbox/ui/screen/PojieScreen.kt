...
fun PojieScreen(onMenuClick: () -> Unit) {
    ...
    val pages = remember(pojieSettings, pojieWifiController) {
        listOf(
            object : NavPage {
                override val name = "资源"; override val selectedIcon = Icons.Filled.Inbox; override val unselectedIcon = Icons.Outlined.Inbox
                override val content = @Composable { ResourcesPage() }
            },
            object : NavPage {
                override val name = "历史"; override val selectedIcon = Icons.Filled.History; override val unselectedIcon = Icons.Outlined.History
                override val content = @Composable { HistoryPage() }
            },
            object : NavPage {
                override val name = "运行"; override val selectedIcon = Icons.Filled.Home; override val unselectedIcon = Icons.Outlined.Home
            ...
            object : NavPage {
                override val name = "设置"; override val selectedIcon = Icons.Filled.Settings; override val unselectedIcon = Icons.Outlined.Settings
                override val content = @Composable { SettingsPage(pojieSettings) { pojieSettings = it } }
            },
            object : NavPage {
                override val name = "帮助"; override val selectedIcon = Icons.Filled.Info; override val unselectedIcon = Icons.Outlined.Info
        ...
        if (showPasswordSheet) {
            ...
            ModalBottomSheet(onDismissRequest = { showPasswordSheet = false }, sheetState = sheetState) {
                ...
                Column(Modifier.fillMaxWidth().padding(16.dp).navigationBarsPadding()) {
                    Text("输入密码本 - $currentTargetSsid", style = MaterialTheme.typography.headlineMedium)
                    OutlinedTextField(
                        value = passwordInputText,
                        onValueChange = { passwordInputText = it; isError = false },
                        isError = isError,
                        supportingText = { if (isError) Text("请输入至少一个密码", color = MaterialTheme.colorScheme.error) },
                        label = { Text("密码本") },
                        modifier = Modifier.fillMaxWidth().weight(1f, false).padding(vertical = 16.dp).heightIn(min = 200.dp),
